...
func SetupRoutes(echo *echo.Echo) map[string]interface{} {
	...
	g.POST("/:id/build_task", gae_support.With(h.idToPl(h.PlToOrg(withAuth(h.pipelineTask("build"))))))
	g.PUT("/:id/close", h.Actions["close"])
	// g.POST("/:id/close_task", h.Actions["close_task"])
	g.POST("/:id/close_task", gae_support.With(h.idToPl(h.PlToOrg(withAuth(h.pipelineTask("close"))))))
